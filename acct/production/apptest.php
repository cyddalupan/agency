...
?>
  ...
  <div class="row">
	...
	<th>Agency</th>
	<th>Status</th>
	<th STYLE="width:12%">PHP Credit</th>
		<th STYLE="width:12%">USD Credit</th>
...
$app = mysql_query("SELECT applicant_id,applicant_first,
 applicant_last,applicant_source,applicant_employer,applicant_date_applied,applicant_preferred_country,applicant_status
 FROM applicant
ORDER BY applicant_id desc limit 20");
...
$app1 = mysql_query("SELECT SUM(amount) as USD ,app_id,currency FROM email_address
WHERE app_id=".$row["applicant_id"]." AND currency='USD'");
$usd=mysql_fetch_array($app1);

$app2 = mysql_query("SELECT SUM(amount) as PHP2 ,app_id,currency FROM email_address
WHERE app_id=".$row["applicant_id"]." AND currency='PHP'");
$php=mysql_fetch_array($app2);


$unfit = mysql_query("SELECT SUM(receivable) as receive ,app_id,receivable FROM email_address4
WHERE app_id='".$row["applicant_id"]."'  AND currency='PHP'");
$unfitrow=mysql_fetch_array($unfit);

$CON = mysql_query("SELECT SUM(receivable) as receiveusd ,app_id,receivable FROM email_address4
WHERE app_id='".$row["applicant_id"]."'  AND  currency='USD' ");
$contract=mysql_fetch_array($CON);

$result1 = mysql_query("SELECT count(app_id) as alls FROM  email_address2 where app_id='".$row['applicant_id']."'");
$all = mysql_fetch_array($result1);


$col = mysql_query("SELECT SUM(amount) as amountusd ,app_id,amount FROM email_address6
WHERE app_id='".$row["applicant_id"]."'  AND  currency='USD' ");
$colusd=mysql_fetch_array($col);


$colS = mysql_query("SELECT SUM(amount) as amounPHP ,app_id,amount FROM email_address6
WHERE app_id='".$row["applicant_id"]."'  AND  currency='PHP' ");
...
echo'<td><a href="add_app_expenses.php?appid='.$row['applicant_id'].'" STYLE="COLOR:BLUE" target="_blank">Add Commission</A></br>
<a href="add_unfit_app.php?appid='.$row['applicant_id'].'" STYLE="COLOR:red" target="_blank">Add Expenses</A>
</td>';
echo'</tr>';
}
?>
