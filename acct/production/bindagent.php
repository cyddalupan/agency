...
<button type="submit" class="btn btn-info"  name="SearchME" >Search FRA</button>
...
echo'<select  class="selectpicker" single data-live-search="true" name="applicant_employer" required style="width:200px">
<option></option>';

$usertype = mysql_query("SELECT * FROM employer  ORDER BY employer_name asc");
echo'<optgroup label="employer">';
while($rowss=mysql_fetch_array($usertype))

{
echo'<option value="'.$rowss["employer_id"].'"> '.$rowss["employer_name"].' - '.$rowss["employer_id"].'</option>';
}
echo'</optgroup>';
...
<button type="submit" class="btn btn-warning btn-sm pull-right"  name="updateapproval" style="border:BORDER 2PX SOLID black;color:white">Bind Data</button>
</a>
...
SET  applicant_employer ='".$_POST['applicant_employer']."' ,singil1=2
...
where
singil1=1
AND applicant_remarks3 LIKE '%".$_POST['search1']."%' ");
...
where  applicant_remarks3='".$row['applicant_remarks3']."'
AND singil1=1");
$countmein1 = mysql_fetch_array($countmein);

echo'<tr style="">';
echo'<td><input type="checkbox" name="delall[]" value="'.$row['applicant_id'].'" checked></td>';

echo'<td>'.$row["applicant_first"].' '.$row["applicant_middle"].'  '.$row["applicant_last"].'</td>';
;
echo'<td>'.$row["applicant_remarks3"].'</td>';
echo'<td>'.$countmein1["alls"].'</td>';
echo'<td>'.$row["singil1"].'</td>';
echo'</tr>';
