...
echo'<td>'; echo date('Y-M-d', strtotime($row['date_added']));echo'</td>';
echo'<td>'.$row["applicant_name"].'</td>';
echo'<td>'; echo date('Y-M-d', strtotime($row['date_hearing']));echo' '.$row['timess'].'</td>';
echo'<td>'.$row1["agent_first"].' '.$row1["agent_last"].'</td>';
echo'<td style="color:Red">'.$row["caso"].' </td>';
echo'<td>'.$row["remarks"].' </td>';
echo'<td><a href="case_view.php?case='.$row['case_id'].'" STYLE="COLOR:BLUE" target="_blank">View</A></td>';
echo'</tr>';
}
?>
</tbody>
			...
			<form class="form-horizontal form-label-left input_mask" method="post" action="">	
					...
					<div class="form-group">
                        ...
                        <div class="col-md-9 col-sm-9 col-xs-12">
                          <input type="date" class="form-control" name="date_added" value="<?=$date1?>" required>
                        </div>
                      </div>
