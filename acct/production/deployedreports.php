...
<form action="r/deployed.php" method="post" target="_blank" >
...
$empS = mysql_query("SELECT * FROM  recruitment_agent order by agent_first asc");
while($rows1emp=mysql_fetch_array($empS))
{
echo'<option value="'.$rows1emp["agent_id"].'"> '.$rows1emp["agent_first"].' '.$rows1emp["agent_last"].'</option>';
}
ECHO'</select>';
?>
</div>
...
<div class="form-group" >
<label class="control-label col-md-3 col-sm-3 col-xs-12">Prinicipal</label>
<div class="col-md-4 col-sm-4 col-xs-12">
<?php
...
<div class="form-group" >
<label class="control-label col-md-3 col-sm-3 col-xs-12">R.O</label>
<div class="col-md-4 col-sm-4 col-xs-12">
	
<?php 
echo'<select class="form-control" name="other_source" >
<option value=0>All</option>';
$usertype1 = mysql_query("SELECT * FROM user 
where user_type NOT IN (4,5)
ORDER BY user_fullname asc");
while($rows1=mysql_fetch_array($usertype1))
{
echo'<option value="'.$rows1["user_name"].'">  '.$rows1["user_fullname"].'</option>';
}
ECHO'</select>';
?>
