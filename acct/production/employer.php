...
<a id="downloadLink" onclick="exportF(this)" style="color:Red;FLOAT:right;font-size:14px;padding:5px">Save my file</a>
...
WHERE WHERE applicant_status IN (25,1) 
AND applicant_source IN  (5,6,8,11,16,40,57,88,97) AND applicant_employer=".$row['employer_id']." ");
...
$singil=$chargejayven+$oldcollect1["amountusd"]+$oldme81["amount"]+$backout1["amount"];
...
if($singil==0){$hidemeup='style="display:none"';}
if($singil!=0){$hidemeup="";}
...
<button type="button" class="btn btn-danger btn-xs" style="font-size:11px;margin:3px">View</button></a>
...
<a href="invoices.php?emp_id='.$row["employer_id"].'" target="_blank" style="color:blue">PRINT INVOICE</a>
...
Else{
echo'<td>
<a href="frareports.php?emp_id='.$row["employer_id"].'" target="_blank">
<button type="button" class="btn btn-danger btn-xs" style="font-size:11px;margin:3px">Debit/Credit</button></a>

<a href="invoices.php?emp_id='.$row["employer_id"].'" target="_blank">Print(2)</a>
</td>';	
}
...
<tr>
<td colspan="11" style="text-align:right"><?=number_format($grandbalance,2)?></td>
</tr>
</table>
