...
$app1 = mysql_query("SELECT * FROM recruitment_agent where agent_id=".$_GET['agent']." ");
...
<a href="profile1.php?agent=<?=$_GET["agent"]?>" style="color:blue;font-size:14px">Back to Profile</a>

</p>
<h1 style="font-size:16px;color:black;text-align:left;padding-left:27px;font-weight:bold">AGENT/SOURCE:  <?=$row1['agent_first']?> <?=$row1['agent_last']?></h1>
...
<h5>Add New Transaction<small style="color;black"> Payment</small></h5>
...
$applicant = mysql_query("SELECT applicant_id,applicant_last,applicant_first,applicant_middle,applicant_source FROM applicant 
where applicant_source=".$_GET['agent']."
ORDER BY applicant_date_applied asc");
while($rows1=mysql_fetch_array($applicant))
{
echo'<option value="'.$rows1["applicant_id"].'">'.$rows1["applicant_last"].'  '.$rows1["applicant_middle"].'  '.$rows1["applicant_first"].'</option>';
}
?>
</select>
</div>
...
<div class="col-md-3 col-sm-3col-xs-12 form-group has-feedback">
...
<input type="hidden" class="form-control" id="inputSuccess5" placeholder="Other charges" name="status_me" value="1"  >
...
<label class="control-label col-md-3 col-sm-3 col-xs-12">Descirption</label>
<div style="clear:both;height:10px"> </div>
<div class="col-md-3 col-sm-6 col-xs-12 form-group has-feedback">
...
<div class="col-md-3 col-sm- col-xs-12 form-group has-feedback">
<?php echo'
...
<h5 style="color:red">List Transaction</h5>
...
$app11 = mysql_query("delete  FROM email_address4 where exp_id= ".$delall)
...
$app = mysql_query("SELECT * FROM email_address4 where  agent_id=".$_GET['agent']."  AND  status_me=1 order by exp_id desc");
...
echo'<tr>';
echo "<td><input type='checkbox' name='delall[]' value='".$row['exp_id']."' ></td> ";
