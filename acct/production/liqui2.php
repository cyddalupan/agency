...
mysql_query("update e_liq SET l_status=1,mami=1
...
$expenses = mysql_query("SELECT * FROM  email_address where e_id=".$row22222['l_e_d']." ");
...
echo'<td>'.$expenses1["e_id"].'</td>';
echo'<td>'.$row22222["l_e_d"].'</td>';
...
<a href="expenses_edit.php?e_id='.$expenses1["e_id"].'" target="_blank"  STYLE="color:white">Review</button></a>
</td>';
...
{
echo'<option value="'.$rows1["applicant_id"].'"> '.$rows1["applicant_first"].' '.$rows1["applicant_last"].' -'.$rows1["applicant_id"].'</option>';
}
ECHO'</select>';
?>
