...
if($_POST['status']==99){
	$status1="Selected/For Booking/For Deployment";
...
if($_POST['keyword']!=''){
	$key="AND position_name  LIKE '%".$_POST['keyword']."%'";
...
$sub_status order by applicant_employer asc
");
$counter = 1; 


//AND position_name  LIKE '%NURSE%'
?>
...
if($userrow['user_fullname']!=''){
	$user=$userrow['user_fullname'];
}
if($userrow['user_fullname']==''){
	$user="Website";
}
		...
		<td STYle="text-transform:uppercase"><?=$row['country_name']?></td>
		...
		<?php if($row['flight_date']=='0000-00-00') { ?>
		<td style="width:140px;text-transform:uppercase"></td>
		<?php } ?>
		<?php if($row['flight_date']!='0000-00-00') { ?>
		<td style="width:140px;text-transform:uppercase"><?php echo date('d-F-Y', strtotime($row['flight_date'])); ?></td>
		<?php } ?>
		<td STYle="text-transform:uppercase"><?=$status?> <small style="color:Red">
		</br><?=$row['sub_status']?></small></b></td>
		<td STYle="text-transform:uppercase"><?=$row['applicant_remarks']?></td>
		<?php
			$counter++;
			}
		?>
