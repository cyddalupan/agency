		...
		<H3 style="font-size:30px;color:black;text-align:center;TEXT-TRANSFORM:UPPERCASE"><?=$row['employer_name']?></H3>
	<?php
	$sum = mysql_query("SELECT SUM(amount) as amountt,amount,agent_id FROM email_address6 where  emp_id=".$_GET['emp_id']."
		AND currency='PHP'
		order by date desc");
	$sumphp=mysql_fetch_array($sum);

	$sumusd = mysql_query("SELECT SUM(amount) as amountt,amount,agent_id FROM email_address6 where  emp_id=".$_GET['emp_id']."
		AND currency='USD'
		order by date desc");
	$sumusd2=mysql_fetch_array($sumusd);	
	
		$appPHP = mysql_query("SELECT SUM(amount_paid) as amountt,amount_paid,agent_id,receivable,currency FROM email_address4 where  emp_id=".$_GET['emp_id']."  AND currency='PHP'
		and receivable!=0
		");	
		$appsump=mysql_fetch_array($appPHP);
	
	
			$appusd = mysql_query("SELECT SUM(amount_paid) as amountt,amount_paid,agent_id,receivable,currency FROM email_address4 where  emp_id=".$_GET['emp_id']."  AND currency='USD'
		and receivable!=0
		");	
...
<?PHP IF($sumphp['amountt']!=''){ ?>
		...
		<tbody>
		<?PHP
		$col = mysql_query("SELECT * FROM email_address6 where  emp_id=".$_GET['emp_id']."
		AND currency='PHP'
		order by date desc");
...
<?PHP IF($sumusd2['amountt']!=''){ ?>
	...
	<?PHP IF($appsump['amountt']!=''){ ?>	
		...
		<tbody>
		<?PHP
		$app = mysql_query("SELECT * FROM email_address4 where  emp_id=".$_GET['emp_id']."  AND currency='PHP'
		and receivable!=0
