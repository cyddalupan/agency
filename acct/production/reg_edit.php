...
$app11 = mysql_query("delete  FROM suppliers where sup_id=".$_GET['e_id']." ");
...
<button type="submit" class="btn btn-danger pull-right"  name="delete">Delete Account</button>
</form>		
</DIV>
<?php }  ?>
...
<h4>Cient/Suppliers :   <b><?=$row["name"]?> </br>
Code # :   <b>000<?=$_GET['e_id']?>

<a href="viewtransaction.php?client=<?=$row['sup_id']?>" onclick="popUp(this.href,'console',700,1200);return false;" target="newWin">
<button type="submit" class="btn btn-warning btn-xs" style="">View Transactions</button></b></a></h4>
...
<input type="text" class="form-control" name="owner" REQUIRED value="<?=$row['owner']?>">
</div>
</div>
...
<div class="col-md-9 col-sm-9 col-xs-12">
<input type="text" class="form-control" name="contact1"  value="<?=$row['contact1']?>">
</div>
</div>
...
<label class="control-label col-md-3 col-sm-3 col-xs-12">CONTACT #</label>
<div class="col-md-9 col-sm-9 col-xs-12">
<input type="text" class="form-control" name="contact2" value="<?=$row['contact2']?>">
</div>
</div>
...
<div class="col-md-9 col-sm-9 col-xs-12">
<input type="text" class="form-control" name="email_address" value="<?=$row['email_address']?>">
</div>
</div>
...
<input type="hidden" class="form-control" name="sup_id" value="<?=$_GET['e_id']?>">
...
VALUES (".$_GET['e_id'].",'".$filename1."','".$_POST['docs_name']."','client' )");
echo'Added';
...
AND file_status='client' order by id desc");
