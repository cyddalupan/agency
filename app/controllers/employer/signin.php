<?php
use \Application\Message as Message;

defined('BASEPATH') OR exit('No direct script access allowed');

class Signin extends Employer_Controller {
	
	const PAGE_ACCESS = parent::PAGE_NOUSER;
	
	public function __construct() 
	{
		parent::__construct();
		
		//make sure page has a settings
		if(!isset($_SESSION["settings"])){
			$this->session->set_flashdata('prev_url', current_url());
			redirect('add_settings');
		}

		if ( isset( $_SESSION['employer']['user'] ) ) {
			redirect( site_url( $this->controllerDir . 'dashboard') );
			exit;
		}
		
		//Check page permission
		$this->checkPageAccess( self::PAGE_ACCESS );

		$this->load->model( $this->modelDir . 'm_signin' ); 
	}
	 
    public function index()
	{	
		if ( isset( $_POST['login'] ) ) {

			$login     = $_POST['login'];
			$loggedIn  = $this->m_signin->login( $login['username'], $login['password'] );
            $returnUri = isset($_GET['return_uri']) ? '?return_uri='.$_GET['return_uri'] : '';
			
			if ( $loggedIn === true ) {
				$user = $_SESSION['employer']['user'];

				Message::addSuccess('Welcome back '.$user['user_fullname'] . '!');
				
                $url = site_url( ! empty( $returnUri ) ? $_GET['return_uri'] : 'employer/dashboard');

			    redirect( $url );
				exit;
			} elseif ( $loggedIn === false ) {
								
				Message::addModalWarning('User not found. Invalid username or password.');
				
                redirect( site_url('employer/signin') . $returnUri);
				exit;
			} elseif ( $loggedIn === 0 ) {
				Message::addDanger('Your user account is deactivated. Please contact administrator regarding this issue.');
				
				redirect( site_url('employer/signin') . $returnUri);
				exit;
			} elseif ( $loggedIn === 1 ) {
				Message::addModalWarning('Your user account is not belong to this system. Please login to this link: <a href="'.site_url('admin/signin').'">'.site_url('employer').'</a>');
				
				redirect( site_url('employer/signin') . $returnUri);
				exit;
			}
		}
		
		$this->setTitle('Sign in to STEPUP MANPOWER', false)
			->renderPage('signin', true); //Full Page
	}
}

/* End of file signin.php */
/* Location: ./app/controllers/employer/signin.php */