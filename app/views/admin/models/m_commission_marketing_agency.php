...
class m_commission_marketing_agency extends MY_Model {
 	/* Constants
	-------------------------------*/
	/* Public Properties
	-------------------------------*/
	...
	public function getAgencies( $onlyUnpaid = true, $from = null, $to = null )
	{
		//Start Transaction
		$this->db->trans_begin();
		...
		$this->db->from( 'marketing_agency' )
			->join( 'commission_marketing_agency', 'commission_agency = agency_id', 'inner' );
		...
		foreach ($agencies as $agencyId => $agency) {
			...
			$agencies[$agencyId] = array_merge( $agencies[$agencyId], $counter, $amount );
		}
	...
	public function getCommissionDetails( $agencyId, $onlyUnpaid = true, $from = null, $to = null )
	{
		$this->db->flush_cache();
		$this->db->from( 'commission_marketing_agency' )
	...
	public function markAsPaid( $commissionId )
	{
        ...
        $voucherData = [
			...
			'voucher_marketing_agency'  => $commission['commission_agency'],
	...
	public function paidAll( $agencyId, $from = null, $to = null )
	{
		...
		$this->db->update( 'commission_marketing_agency', $commissionData );
	...
	public function createCommission( $employerId, $applicantId )
	{
		...
		$contract = ( new m_contract_marketing_agency )->getContractByEmployerId( $employerId );
		...
		$amount     = $contract['contract_placement_fee'] 
		...
		$this->db->flush_cache();
        $this->db->insert( 'commission_marketing_agency', $commissionData );
