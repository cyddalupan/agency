...
class Users extends Admin_Controller {
	...
	public function change_password($oldpass,$newpass)
	{
		 ...
		 $encoldpas = ( new m_user )->encryptPass($oldpass);

		if($_SESSION['admin']['user']['user_password'] == $encoldpas){

			$newEncPass = ( new m_user )->encryptPass($newpass);
			$data = array(
               'user_password' => $newEncPass
            );

			$this->db->where('user_id', $_SESSION['admin']['user']['user_id']);
