...
class Ajax extends Admin_Controller {
	...
	public function categories( $method )
	{
		switch ( $method ) {
			...
			case 'delete':
				...
				$category = ( new m_category )->find( $categoryId );

				( new m_category )->delete( $categoryId );
	...
	public function positions( $method )
	{
		switch ( $method ) {
			case 'delete-position':
				$positionId = $this->segments[5];
				
				$this->load->model( 'm_position' );

				$position = ( new m_position )->find( $positionId );

				( new m_position )->delete( $positionId );
	...
	public function countries( $method )
	{
		switch ( $method ) {
			...
			case 'delete':
				...
				$country = ( new m_country )->find( $countryId );

				( new m_country )->delete( $countryId );
