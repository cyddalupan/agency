...
class Employers extends Admin_Controller {
	...
	public function all()
	{
		...
		if ( isset( $_GET['delete'], $_GET['token'] ) && is_numeric( $_GET['delete'] ) ) {
			if ( $_GET['token'] != session_id() ) {
				echo 'This request is already expired.';
				exit;
			}

			$employerId = intval( $_GET['delete'] );

			$employer = ( new m_employer )->delete( $employerId );

			Message::addInfo( '<strong>'.$employer['employer_name'].'</strong> has been removed from the list.' );

			redirect( site_url( 'admin/employers/all' ) );
			exit;
		}
		
		$employers      = ( new m_employer )->getEmployers( $options, Pagination::getPerPage(), Pagination::getRecordCursor() );
		$employersCount = ( new m_employer )->getEmployersCount( $options ); 
	...
	{	
		...
		if ( isset( $_POST['employer'], $_POST['employer']['user'] ) ) {
			...
			if ( (!empty( $employer )) && (isset($employer['employer_name'])) ) {
