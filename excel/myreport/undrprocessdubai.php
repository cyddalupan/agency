...
where      applicant_status    NOT IN (9,21,22,1,14,25,5,39,43,0,7,15,42)
AND requirement_contract   IN ('1970-01-01','0000-00-00','')
AND applicant_employer=".$_GET["empid"]." 

group by applicant_id order by status asc");
...
IF($row['requirement_contract']=='1970-01-01' || $row['requirement_contract']=='0000-00-00'  || $row['requirement_contract']=='') { echo"<td></td>"; }
else{  echo'<td>'; echo date('M-d-Y', strtotime($row['requirement_contract']));echo'</td>'; }

IF($row['requirement_contract_sign']=='1970-01-01' || $row['requirement_contract_sign']=='0000-00-00' || $row['requirement_contract_sign']=='' ) { echo"<td></td>"; }
