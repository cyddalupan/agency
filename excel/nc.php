		...
		* {
		...
		<a class="brand" href="#">Imporst Excel WELFARE</a>
		...
		<li><a class="active" href="nc.php">Pending</a></li>
	...
	<table class="js-sort-table" style="width:110%;margin-left:-30px" id="myTable">
	<thead>
	<tr>
	<th></th>		  	    
	<th></th>			  	    
	...
	<th>Date Deployed</th>			
		...
		where app_status IN ('FOLLOW UP','WAITING RESPONSE','NC','')order by SUBJ_ID desc ";
		...
		LEFT JOIN recruitment_agent
		ON subject.agent_id = recruitment_agent.agent_id
		where  fra_name='".$_POST['fra']."' and app_status IN ('FOLLOW UP','WAITING RESPONSE','NC','') order by SUBJ_ID desc ";
		...
		where  app_country='".$_POST['country']."' AND  app_status IN ('FOLLOW UP','WAITING RESPONSE','NC','') order by SUBJ_ID desc ";
		...
		<?php echo'<tr style="background:'.$color111a.';color:'.$color21.'">'; ?>
                ...
                <?php echo'<td>'; echo date('M-d-Y', strtotime($row['app_last']));echo'</td>'; ?>
				
 <?php echo'<td>'; echo date('M-d-Y', strtotime($row['app_deployed']));echo'</td>'; ?>
