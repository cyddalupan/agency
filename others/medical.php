...
<h3 style="color:Red"><span> <?=$stat?> Applicants</span></h3>
<!-- para -->
...
<?php
include'db.php';
$app = mysql_query("SELECT applicant_certificate.*,applicant.* FROM applicant
LEFT JOIN applicant_certificate ON applicant.applicant_id = applicant_certificate.certificate_applicant
where applicant_status=".$_GET['id']."
...
<a target="_blank"  href="http://stepupmanpower.com/admin2019/admin/applicants/review_single/';?><?=$row['applicant_id']?>

<?php	
echo'" class="btn btn-xs btn-danger">Review</a>
</td>  ';	
...
echo"<td>".$row['certificate_medical_result']." </td>";
echo"<td>".$row['certificate_mmr']." </td>";
