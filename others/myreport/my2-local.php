...
<h1 style="text-align:center">LOCAL FLIGHT </h1>
...
where localflight NOT IN ('1970-01-01','0000-00-00','')

AND localflight='".$_GET["dateme"]."' 
AND branch_type=".$_GET['imy']." group by applicant_id");
...
echo'<td>'; echo date('M-d-Y', strtotime($row['vfs']));echo'</td>';

IF($row['certificate_owwa_from']=='1970-01-01' || $row['certificate_owwa_from']=='0000-00-00' || $row['certificate_owwa_from']=='') { echo"<td></td>"; }
else{  echo'<td>'; echo date('M-d-Y', strtotime($row['certificate_owwa_from']));echo'</td>'; }
...
IF($row['localflight']=='1970-01-01' || $row['localflight']=='0000-00-00' ) { echo"<td></td>"; }
else{ echo"<td>".$row['localflight']."</td>"; }
