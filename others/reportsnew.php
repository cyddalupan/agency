...
<h1 style="color:black"><span>  GENERAL REPORTS</span></h1>
<!-- para -->
...
<li><a href="booking.php">Deployment</a></li>
...
<body>
<P style="color:red"> if start FILTER date is null or blank automatic all</P>

<select id='purpose' style="border:1px solid black;padding:5px;width:300px">
<label for="cars">Choose a Reports:</label>
...
<option value="9">SIGNING CONTRACT</option>
...
<form method="post" class="form" id="owwame" style='display:none;'  action="<?=$myreport?>onowwa.php?user=<?=$_GET['user']?>" target="_blank" style="padding:10px;border:1px solid lightgray;margin:2px;WIDTH:600PX;float:left;margin:10px">
<H2>OWWA </H2>
<div class="row">
...
<form method="post" class="form" id="tesdame" style='display:none;'  action="<?=$myreport?>ontesda.php?user=<?=$_GET['user']?>" target="_blank" style="padding:10px;border:1px solid lightgray;margin:2px;WIDTH:600PX;float:left;margin:10px">
<H2>TESDA </H2>
<div class="row">
...
<form method="post" class="form" id="oecme1" style='display:none;'  action="<?=$myreport?>onprocessexcel1.php?user=<?=$_GET['user']?>" 
...
<input type="hidden" value="31" name="status_app">
<input type="hidden" value="99999" name="sub_status">
<div class="row">
<div class="col-sm-3">
Date From/To<small><em>(optional)</em></small>:
</div>
...
<form method="post" class="form" id="onprocess" style='display:none1;'  action="<?=$myreport?>onprocessexcel1.php?user=<?=$_GET['user']?>" 
target="_blank" style="padding:10px;border:1px solid lightgray;margin:2px;WIDTH:200PX;float:left;margin:10px;border:2px solid gray">
<h2>On process</h2>
<div class="row">
...
<input type="hidden" value="2" name="status_app">
<input type="hidden" value="99999" name="sub_status">
<div class="row">
<div class="col-sm-3">
Date From/To<small><em>(optional)</em></small>:
...
<div class="col-sm-5">
<select name="typess" class="form-control" required>
<option value="99999">-- ALL BRANCH --</option>
<option VALUE="1">MAIN MANILA  BRANCH</option><option VALUE="2">ILOILO</option><option VALUE="3">BACOLOD</option><option VALUE="4">DAVAO</option><option VALUE="5">GENSAN</option><option VALUE="6">TUGUEGARAO</option><option VALUE="7">PAGADIAN</option><option VALUE="8">KORONADAL</option>
</select>
</div>
</div>
...
<?php
echo'<select class="form-control" name="applicant_status" onchange="showapplicant(this.value)" required>';
echo'<option value="11111"> -All Status-</option>';
$stats = mysql_query("SELECT * FROM statuses ORDER BY number asc");
while($stats1=mysql_fetch_array($stats))
{
echo'<option value="'.$stats1["number"].'">  '.$stats1["status"].'</option>';
}
ECHO'</select>';
?>
</div>
...
<?php
echo'<select class="form-control" name="applicant_preferred_country" onchange="showapplicant(this.value)" required>';
echo'<option value="11111"> -All Status-</option>';
$app12= mysql_query("SELECT * FROM  country");
while($ccountry=mysql_fetch_array($app12))
{
echo'<option value="'.$ccountry["country_id"].'">  '.$ccountry["country_name"].'</option>';
}
ECHO'</select>';
?>
</div>
...
<form method="post" class="form" id="onbooking" style='display:none;'  action="<?=$myreport?>ondeployed1.php?book=2" 
target="_blank" style="padding:10px;border:1px solid lightgray;margin:2px;WIDTH:200PX;float:left;margin:10px;border:2px solid gray">
<h2>BOOKING</h2>
<div class="row">
...
<form method="post" class="form" id="ondeployed" style='display:none;'  action="<?=$myreport?>ondeployed1.php?book=1" 
target="_blank" style="padding:10px;border:1px solid lightgray;margin:2px;WIDTH:200PX;float:left;margin:10px;border:2px solid gray">
<h2>DEPLOYMENT</h2>
<div class="row">
...
<option value="3">Signing Contract </option>
