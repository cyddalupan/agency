...
<h1 STYLE="COLOR:BLACK"><span><span> Visa </span></h1>
<!-- para -->

<?php
include'db.php';
include'functions_visa.php';
$app = mysql_query("SELECT applicant_certificate.*,applicant.*,statuses.*,applicant_requirement.*
FROM applicant
LEFT JOIN applicant_certificate ON 
applicant.applicant_id = applicant_certificate.certificate_applicant

LEFT JOIN applicant_requirement ON 
applicant.applicant_id = applicant_requirement.requirement_applicant

LEFT JOIN  statuses
ON applicant.applicant_status = statuses.number

where applicant_status IN (4,2,3,5,8,12,26,27,28,29,30,31)
and requirement_visa_no=''
...
<li><a   class="active" href="stampnew.php"><?=$visamenu?></a></li>
...
<p>NOTE: If Visa No. not NULL/empty it will remove to the list</p>
...
echo"<td>".$row2['applicant_requirement_ecode']."</td>";
echo"<td>".$row2['applicant_requirement_paid']."</td>";
IF($row2['vfs']=='1970-01-01' || $row2['vfs']=='0000-00-00' ||  $row2['vfs']=='0001-01-01' || $row2['vfs']=='' ) {
echo"<td></td>";
}else{  echo'<td>'; echo date('M-d-Y', strtotime($row2['vfs']));echo'</td>'; }

echo"<td>".$row2['applicant_requirement_kawala']."</td>";
...
<input type='TEXT'  name='certificate_visa_remarks[]' value='".$row1['certificate_visa_remarks']."'></td>";
