...
IF($_GET['num']==1){
$title="Today";
$filter="AND date_update between date_sub(now(), interval 0 day)  AND date_add(now(), interval 1 day)";

}
IF($_GET['num']==2){
$title="Weekly";
$filter="AND date_update between date_sub(now(), interval 7 day)  AND date_add(now(), interval 1 day)";
}
IF($_GET['num']==3){
$title="Monthly";
$filter="AND date_update between date_sub(now(), interval 30 day)  AND date_add(now(), interval 1 day)";
}
include'../db.php';
...
<h1 style="text-align:center"><?=$title?> Close Case/Complaint</h1>
...
<th style="width:3%">#</th>
<th>NAME</th>
<th style="width:12%">DATE ISSUE</th>
...
order by date_added desc ");
			...
			echo'<td>'.$count.'</td>';
			if($row['app_id']==0){
			echo'<td>'.$row["lname"].', '.$row["applicant_name"].' '.$row["mname"].'</A></b>';echo'&nbsp; </td>';}

			if($row['app_id']!=0){echo'<td> '.$approw["applicant_last"].', '.$approw["applicant_first"].'';echo'&nbsp; </td>';}
