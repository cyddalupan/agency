...
IF($_GET['num']==1){
$title="POEA";
$filter="AND case_type IN ('POEA')";

}
IF($_GET['num']==2){
$title="OWWA";
$filter="AND case_type NOT IN ('OWWA')";
}
IF($_GET['num']==3){
$title="LABOUR";
$filter="AND case_type NOT IN ('LABOUR')";
}

IF($_GET['num']==4){
$title="ARMM";
$filter="AND case_type NOT IN ('ARMM')";
}


IF($_GET['num']==5){
$title="FB/PRO/AGENT/SMS/WALK-IN/OTHERS";
$filter="AND case_type NOT IN ('POEA','OWWA','LABOUR','ARMM')";
}
include'../db.php';
...
$approw=mysql_fetch_array($app1);
			...
			<a href="'.$review.''.$approw['applicant_id'].'"  target="_blank">
