...
IF($_GET['num']==1){
$title="Today";
$filter="AND medical_fit between date_sub(now(), interval 0 day)  AND date_add(now(), interval 1 day)";

}
IF($_GET['num']==2){
$title="Weekly";
$filter="AND medical_fit between date_sub(now(), interval 7 day)  AND date_add(now(), interval 1 day)";
}
IF($_GET['num']==3){
$title="Monthly";
$filter="AND medical_fit between date_sub(now(), interval 30 day)  AND date_add(now(), interval 1 day)";
}

IF($_GET['num']==4){
$title="Yesterday";
$filter="AND medical_fit=".$dateko."";
}
...
<table id="customers" >
<tr>
<td colspan="18"><h2 style="text-align:center"><?=$title?> FTW MEDICAL </h2></td>
<tr>
...
IF($row['medical_fit']=='1970-01-01' || $row['medical_fit']=='0000-00-00' || $row['medical_fit']=='') { echo"<td></td>"; }
else{  echo'<td>'; echo date('M-d-Y', strtotime($row['medical_fit']));echo'</td>'; }
