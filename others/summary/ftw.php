...
IF($_GET['num']==1){
$title="Today";
$filter="created_at between date_sub(now(), interval 0 day)  AND date_add(now(), interval 1 day)";

}
IF($_GET['num']==2){
$title="Weekly";
$filter="created_at between date_sub(now(), interval 7 day)  AND date_add(now(), interval 1 day)";
}
IF($_GET['num']==3){
$title="Monthly";
$filter="created_at between date_sub(now(), interval 30 day)  AND date_add(now(), interval 1 day)";
}

IF($_GET['num']==4){
$title="Yesterday";
$filter="multiple_lineups.created_at=".$dateko."";
}
...
<h1 style="text-align:center"><?=$title?> CV SENT</h1>
...
$groupee =mysql_query("SELECT * FROM multiple_lineups
LEFT JOIN applicant
...
order by applicant.applicant_status
");
...
<?php
$count=1;
$fit =mysql_query("SELECT * FROM multiple_lineups
LEFT JOIN applicant
ON multiple_lineups.applicant_id = applicant.applicant_id
LEFT JOIN  statuses
ON applicant.applicant_status = statuses.number
...
group by multiple_lineups.applicant_id
...
");
			...
			echo'<td>'; echo date('M-d-Y', strtotime($row['created_at']));echo'</td>';
