...
IF($_GET['num']==1){
$title="Today";
$filter="applicant_date_applied between date_sub(now(), interval 0 day)  AND date_add(now(), interval 1 day)";

}
IF($_GET['num']==2){
$title="Weekly";
$filter="applicant_date_applied between date_sub(now(), interval 7 day)  AND date_add(now(), interval 1 day)";
}
IF($_GET['num']==3){
$title="Monthly";
$filter="applicant_date_applied between date_sub(now(), interval 30 day)  AND date_add(now(), interval 1 day)";
}
IF($_GET['num']==4){
$title="Yesterday";
$filter="applicant_date_applied=".$dateko." ";
}
...
<table id="customers" >
<tr>
<td colspan="18"><h2 style="text-align:center"><?=$title?> Medical</h2></td>
<tr>
...
IF($row['certificate_medical_exam_date']=='1970-01-01' || $row['certificate_medical_exam_date']=='0000-00-00' || $row['certificate_medical_exam_date']=='' ) { echo"<td></td>"; }
