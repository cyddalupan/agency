...
IF($_GET['num']==1){
$title="Today";
$filter="AND requirement_oec_release_date between date_sub(now(), interval 0 day)  AND date_add(now(), interval 1 day)";

}
IF($_GET['num']==2){
$title="Weekly";
$filter="AND requirement_oec_release_date between date_sub(now(), interval 7 day)  AND date_add(now(), interval 1 day)";
}
IF($_GET['num']==3){
$title="Monthly";
$filter="AND requirement_oec_release_date between date_sub(now(), interval 30 day)  AND date_add(now(), interval 1 day)";
}

IF($_GET['num']==4){
$title="Yesterday";
$filter="AND  requirement_oec_release_date=".$dateko."";
}
...
<h1 style="text-align:center"><?=$title?> OEC</h1>
...
LEFT JOIN  	employer
ON applicant.applicant_employer =employer.employer_id
where applicant.applicant_employer=".$row1['applicant_employer']."
$filter
");
