{{-- Tab 1b: Cash Advance — agent-charged expense items on Cash Advance accounts --}}
<div class="card bg-base-100 shadow-md">
    <div class="card-body">
        <h3 class="font-bold mb-3">💰 Cash Advance <span class="badge badge-sm badge-ghost">{{ $cashAdvanceItems->count() }}</span></h3>
        @if($cashAdvanceItems->count())
            <div class="overflow-x-auto">
                <table class="table table-sm">
                    ...
                    <tbody>
                        @foreach($cashAdvanceItems as $item)
                            @php
        ...
        @else
            <p class="opacity-50 text-sm py-6 text-center">No cash advance transactions yet.</p>
        @endif
    </div>
</div>
