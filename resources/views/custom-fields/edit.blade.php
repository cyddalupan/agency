...
<div class="max-w-xl mx-auto">
    ...
    <div class="card bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20 mb-6 p-4">
        <h2 class="text-2xl font-bold flex items-center gap-2">
            <span>✏️</span> Edit Custom Field
        </h2>
        <p class="opacity-60 text-sm mt-1">{{ $customField->name }}</p>
    </div>
    ...
    <form action="{{ route('custom-fields.update', $customField) }}" method="POST" class="card bg-base-100 shadow-sm card-lift">
        <div class="card-body space-y-4">
            ...
            <fieldset class="fieldset">
                ...
                <select name="model_type" required class="select w-full">
                    <option value="Employer" @selected(old('model_type', $customField->model_type) === 'Employer')>🏢 Employer</option>
                    <option value="Applicant" @selected(old('model_type', $customField->model_type) === 'Applicant')>👤 Applicant</option>
                    <option value="Bill" @selected(old('model_type', $customField->model_type) === 'Bill')>🧾 Bill</option>
                </select>
            ...
            <fieldset class="fieldset">
                ...
                <input type="text" name="name" value="{{ old('name', $customField->name) }}" required
                    class="input w-full" placeholder="e.g. TIN Number">
            ...
            <fieldset class="fieldset">
                ...
                <select name="type" required class="select w-full">
                    <option value="text" @selected(old('type', $customField->type) === 'text')>🔤 Text</option>
                    <option value="textarea" @selected(old('type', $customField->type) === 'textarea')>📝 Textarea</option>
                    <option value="number" @selected(old('type', $customField->type) === 'number')>🔢 Number</option>
                    <option value="date" @selected(old('type', $customField->type) === 'date')>📅 Date</option>
                    <option value="select" @selected(old('type', $customField->type) === 'select')>📋 Dropdown (Select)</option>
                    <option value="checkbox" @selected(old('type', $customField->type) === 'checkbox')>✅ Checkbox</option>
                    <option value="url" @selected(old('type', $customField->type) === 'url')>🔗 URL</option>
                </select>
                @error('type') <p class="text-error text-sm mt-1">{{ $message }}</p> @enderror
            </fieldset>

            <fieldset class="fieldset" id="options-fieldset">
                <legend class="fieldset-legend">📋 Options</legend>
                <textarea name="options" class="textarea w-full" rows="5"
                    placeholder="One option per line:&#10;Option 1&#10;Option 2">{{ old('options', is_array($customField->options) ? implode("\n", $customField->options) : '') }}</textarea>
                <p class="text-xs opacity-50 mt-1">Enter each option on a new line (for dropdown fields)</p>
                ...
                <fieldset class="fieldset">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <input type="checkbox" name="required" value="1" class="checkbox checkbox-sm"
                            @checked(old('required', $customField->required))>
                        <span class="text-sm">Make this field mandatory</span>
                    </label>
                </fieldset>
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">🔢 Order</legend>
                    <input type="number" name="order" value="{{ old('order', $customField->order) }}"
