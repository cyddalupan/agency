...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <a href="{{ route('employer.job-positions.create') }}" class="btn btn-primary">
                    ...
                    <tr class="hover transition-colors">
                        <td>
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('employer.job-positions.show', $job) }}" class="link link-primary font-medium">{{ $job->name }}</a>
                            </div>
                        </td>
                        <td class="text-sm">
                            @if ($job->salary)
                        ...
                        <td class="text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('employer.job-positions.show', $job) }}" class="btn btn-ghost btn-xs btn-square" title="View">👁️</a>
                                <a href="{{ route('employer.job-positions.edit', $job) }}" class="btn btn-ghost btn-xs btn-square" title="Edit">✏️</a>
                                <form action="{{ route('employer.job-positions.destroy', $job) }}" method="POST" class="inline" onsubmit="return confirm('🗑️ Delete this position?')">
                                    @csrf @method('DELETE')
