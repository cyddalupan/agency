...
<div class="max-w-xl mx-auto">
    ...
    <div class="card bg-gradient-to-br from-red-500/10 to-orange-500/10 border border-red-500/20 mb-6 p-4">
        ...
        <p class="opacity-60 text-sm mt-1">Recorded {{ $expense->date->format('M d, Y') }} · ₱ {{ number_format($expense->amount, 2) }}</p>
    </div>
    ...
    <form action="{{ route('expenses.update', $expense) }}" method="POST" class="card bg-base-100 shadow-sm card-lift">
        <div class="card-body space-y-4">
                        ...
                        <optgroup label="{{ $main->name }}">
                            @if($main->children->count())
                                @foreach($main->children as $sub)
                                    <option value="{{ $sub->id }}" @selected(old('account_id', $expense->account_id) == $sub->id)>{{ $sub->name }}</option>
                                @endforeach
                            @else
                                <option value="{{ $main->id }}" @selected(old('account_id', $expense->account_id) == $main->id)>{{ $main->name }}</option>
                            @endif
                        </optgroup>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <fieldset class="fieldset">
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" value="{{ old('amount', $expense->amount) }}" required class="input w-full">
                ...
                <fieldset class="fieldset">
                    ...
                    <input type="date" name="date" value="{{ old('date', $expense->date->toDateString()) }}" required class="input w-full">
            ...
            <fieldset class="fieldset">
                ...
                <input type="text" name="payee" value="{{ old('payee', $expense->payee) }}" class="input w-full">
                ...
                <fieldset class="fieldset">
                    ...
                    <select name="method" class="select w-full">
                        @foreach(['cash','bank_transfer','check','gcash','online'] as $m)
                            <option value="{{ $m }}" @selected(old('method', $expense->method) === $m)>{{ ucwords(str_replace('_',' ',$m)) }}</option>
                        @endforeach
                    </select>
                </fieldset>
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">🔖 Reference No.</legend>
                    <input type="text" name="reference_no" value="{{ old('reference_no', $expense->reference_no) }}" class="input w-full">
            ...
            <fieldset class="fieldset">
                ...
                <textarea name="notes" rows="3" class="textarea w-full">{{ old('notes', $expense->notes) }}</textarea>
            </fieldset>
