...
@section('title', __('messages.line_up'))
...
<script>
function selectApplicant(btn, applicantName) {
    btn.disabled = true;
    btn.textContent = '{{ __('messages.selecting') }}';
    setTimeout(function() {
...
<div>
    {{-- Header --}}
    <div class="lc-header">
        <h1>{{ __('messages.line_up') }}</h1>
        <a href="{{ route('fra.lineup.export') }}" class="lc-export-btn">
    ...
    <div class="lc-filter-group" role="group" aria-label="filter">
        ...
        @foreach($applicants->pluck('position')->filter()->unique('id') as $pos)
            ...
            <a href="{{ route('fra.lineup', ['position' => $pos->name]) }}"
        ...
        <table class="lc-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $applicant->medical_status ?? '—' }}</td>
                    ...
                    <td>
                        <div class="lc-actions">
                            <form method="POST" action="{{ route('fra.lineup.select', $applicant) }}" style="display:inline">
                                @csrf
                                <button type="button" class="btn-select" onclick="selectApplicant(this, '{{ addslashes($fullName) }}')">{{ __('messages.select') }}</button>
                            </form>
                ...
                @empty
                <tr>
                    <td colspan="10" class="lc-empty">{{ __('messages.no_applicants_lineup') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
