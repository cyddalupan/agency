...
@section('title', __('messages.selected'))
...
function submitBulkRemove() {
    ...
    if (checked.length === 0) { alert('Select at least one applicant.'); return; }
    if (confirm('Remove ' + checked.length + ' applicant(s) from Selected?')) {
...
<div>
    ...
    <div class="lc-filter-group" role="group" aria-label="filter">
        ...
        @foreach($applicants->pluck('position')->filter()->unique('id') as $pos)
            ...
            <a href="{{ route('fra.selected', ['position' => $pos->name]) }}"
    ...
    {{-- Table --}}
    <form method="POST" action="{{ route('fra.selected.bulk-remove') }}" id="bulk-remove-form">
        @csrf
    <div class="lc-table-wrap">
        <table class="lc-table">
            <thead>
                <tr>
                    <th><input type="checkbox" id="select-all" onchange="toggleAllCheckboxes(this)"></th>
            ...
            <tbody>
                ...
                @endphp
                <tr>
                    <td><input type="checkbox" name="applicant_ids[]" value="{{ $applicant->id }}" class="row-checkbox"></td>
                    <td>
                ...
                @empty
                <tr>
                    <td colspan="9" class="lc-empty">{{ __('messages.no_applicants_lineup') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div style="margin-top:10px;">
        <button type="button" onclick="submitBulkRemove()" class="btn-select" style="background:#ef4444;">
            ❌ Remove Selected
        </button>
    </div>
</div>
