...
@section('title', 'Issue Official Receipt')
...
<div class="max-w-xl mx-auto">
    ...
    <div class="card bg-gradient-to-br from-amber-600/10 to-yellow-600/10 border border-amber-500/20 mb-6 p-4">
        <h2 class="text-2xl font-bold flex items-center gap-2">
            <span>➕</span> Issue Official Receipt
        </h2>
        <p class="opacity-60 text-sm mt-1">Generate an official receipt for a confirmed payment</p>
    </div>
    ...
    <form action="{{ route('official-receipts.store') }}" method="POST" class="card bg-base-100 shadow-sm card-lift">
        <div class="card-body space-y-4">
            ...
            <fieldset class="fieldset">
                ...
                <input type="date" name="issue_date" value="{{ old('issue_date', date('Y-m-d')) }}" required class="input w-full">
            </fieldset>
            ...
            <fieldset class="fieldset">
                ...
                <select name="issued_to" required class="select w-full">
                    ...
                    <option value="employer" @selected(old('issued_to') === 'employer')>Employer</option>
                    <option value="applicant" @selected(old('issued_to') === 'applicant')>Applicant</option>
                    <option value="agent" @selected(old('issued_to') === 'agent')>Agent</option>
                </select>
