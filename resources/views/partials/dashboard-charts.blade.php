...
<div class="card bg-base-100 shadow-sm">
    <div class="card-body">
        <h3 class="card-title text-lg mb-4">📋 Active Status Pipeline</h3>
...
(function() {
    ...
    // ===== Pipeline Status Bar (horizontal) =====
    (function pipelineBar() {
        ...
        const y = d3.scaleLinear().domain([0, d3.max(data, d => d.count) * 1.2 || 5]).range([innerH, 0]);
        svg.selectAll('.bar').data(data).join('rect')
            .attr('x', d => x(d.label)).attr('width', x.bandwidth())
            .attr('y', d => y(d.count)).attr('height', d => innerH - y(d.count))
            .attr('fill', d => d.color).attr('rx', 4).attr('opacity', 0.85);
        svg.selectAll('.bar-label').data(data).join('text')
            .attr('x', d => x(d.label) + x.bandwidth() / 2)
            .attr('y', d => y(d.count) - 6)
            .text(d => d.count).attr('text-anchor', 'middle').attr('font-size', '11px').attr('fill', '#666');
        svg.append('g').call(d3.axisLeft(y).ticks(4)).selectAll('text').attr('font-size', '10px').attr('fill', '#999');
        svg.append('g').attr('transform', `translate(0,${innerH})`)
            .call(d3.axisBottom(x)).selectAll('text')
            .attr('font-size', '9px').attr('fill', '#999').attr('transform', 'rotate(-20)');
        svg.selectAll('.domain').attr('stroke', '#ddd');
    })();
