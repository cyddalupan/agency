...
<div class="max-w-xl mx-auto">
    ...
    <div class="card bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20 mb-6 p-4">
        <h2 class="text-2xl font-bold flex items-center gap-2">
            <span>✏️</span> Edit Payment #{{ $payment->id }}
        </h2>
        <p class="opacity-60 text-sm mt-1">Update payment details</p>
    </div>
    ...
    <form action="{{ route('payments.update', $payment) }}" method="POST" class="card bg-base-100 shadow-sm card-lift">
        <div class="card-body space-y-4">
            ...
            <fieldset class="fieldset">
                ...
                <select name="bill_id" class="select w-full">
                    ...
                    @foreach($bills as $bill)
                        <option value="{{ $bill->id }}" @selected(old('bill_id', $payment->bill_id) == $bill->id)>
                            #{{ $bill->id }} · {{ $bill->employer->name ?? 'No employer' }}
                        </option>
                    @endforeach
                </select>
            </fieldset>
            ...
            <fieldset class="fieldset">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="amount" value="{{ old('amount', $payment->amount) }}"
                        required step="0.01" min="0.01" class="input w-full pl-8" placeholder="0.00">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <fieldset class="fieldset">
                    ...
                    <select name="category" required class="select w-full">
                        <option value="employer_cost" @selected(old('category', $payment->category) === 'employer_cost')>Employer Cost</option>
                        <option value="applicant_cost" @selected(old('category', $payment->category) === 'applicant_cost')>Applicant Cost</option>
                        <option value="deposit" @selected(old('category', $payment->category) === 'deposit')>Deposit</option>
                        <option value="commission" @selected(old('category', $payment->category) === 'commission')>Commission</option>
                    </select>
                ...
                <fieldset class="fieldset">
                    ...
                    <select name="type" class="select w-full">
                        ...
                        <option value="cash" @selected(old('type', $payment->type) === 'cash')>Cash</option>
                        <option value="bank_transfer" @selected(old('type', $payment->type) === 'bank_transfer')>Bank Transfer</option>
                        <option value="check" @selected(old('type', $payment->type) === 'check')>Check</option>
                        <option value="gcash" @selected(old('type', $payment->type) === 'gcash')>GCash</option>
                        <option value="online" @selected(old('type', $payment->type) === 'online')>Online</option>
                    </select>
                </fieldset>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <fieldset class="fieldset">
                    ...
                    <input type="date" name="payment_date"
                        value="{{ old('payment_date', $payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('Y-m-d') : date('Y-m-d')) }}"
                        class="input w-full">
                </fieldset>
            </div>
            ...
            <fieldset class="fieldset">
                ...
                <select name="status" class="select w-full">
                    ...
                    <option value="confirmed" @selected(old('status', $payment->status) === 'confirmed')>✅ Confirmed</option>
            ...
            <fieldset class="fieldset">
                ...
                <textarea name="notes" class="textarea w-full" rows="3" placeholder="Optional notes...">{{ old('notes', $payment->notes) }}</textarea>
            </fieldset>
