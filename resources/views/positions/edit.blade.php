...
<div class="max-w-xl mx-auto">
    ...
    <div class="card bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20 mb-6 p-4">
        <h2 class="text-2xl font-bold flex items-center gap-2">
            <span>✏️</span> Edit Position
        </h2>
        <p class="opacity-60 text-sm mt-1">{{ $position->name }}</p>
    </div>
    ...
    <form action="{{ route('positions.update', $position) }}" method="POST" class="card bg-base-100 shadow-sm card-lift">
        <div class="card-body space-y-4">
            ...
            <fieldset class="fieldset">
                ...
                <textarea name="description" class="textarea w-full" rows="4"
                    placeholder="Describe this position...">{{ old('description', $position->description) }}</textarea>
